...
parser = argparse.ArgumentParser(description="LeafGuard webcam classifier")
...
network = jetson_inference.imageNet(
    model="/opt/jetson-inference/python/training/classification/models/leafguard_v2/resnet18.onnx",
    labels="/opt/jetson-inference/python/training/classification/data/leafguard_v2/labels.txt",
    input_blob="input_0",
...
while True:
    ...
    text = f"LeafGuard: {label} ({confidence * 100:.1f}%)"
